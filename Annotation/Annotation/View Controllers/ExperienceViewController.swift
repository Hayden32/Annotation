...
class ExperienceViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, AVAudioPlayerDelegate, AVAudioRecorderDelegate, UITextFieldDelegate {
    ...
    private func applyImageFilter(to image: UIImage) -> UIImage {
        ...
        guard let outputImage = filterEffect?.outputImage else { return image }
        guard let cgImages = context.createCGImage(outputImage, from: outputImage.extent) else { return image }
    ...
    func saveImage(image: UIImage) -> URL {
        guard let documentDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else { fatalError("No access to dcuments directory at this time.") }
